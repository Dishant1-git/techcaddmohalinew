...
export default function FaqFormPage() {
  ...
  /*
    The headings come from the Categories section, not from a list private to
    FAQs.

    There are two category tables — `categories`, which the Categories screen
    manages, and `faq_categories`, which only ever appeared inside this form.
    Reading the private one meant a category created in the CMS never showed up
    here, and the two lists drifted apart. Reading the Categories section makes
    that screen the single place headings are managed: add one there and it is
    offered here immediately.

    The API still stores the choice as a name, and finds-or-creates its own row
    from it, so nothing about the saved record changes.
  */
          ...
          <Card flush>
            ...
            <CardBody className="space-y-5">
              ...
              <FormField
                label="Category"
                required
                description="Headings come from the Categories section. Add one there and it appears here."
              ...
              >
                <Select
                  ...
                  placeholder={
                    categoryOptions.length ? undefined : 'No categories yet — add one first'
                  }
                />
              </FormField>
