...
/**
 * The AI menu's RAG (Retrieval-Augmented Generation) page, at
 * `/courses/ai/rag`.
 *
 * One of the courses whose record does not come from the catalogue in
 * `@/lib/courses`. It belongs to the AI menu alone — the Courses, Certificate
 * Programs and After 12th routes never serve this slug — so the copy below
 * appears on exactly one page.
 *
 * Everything here is the supplied copy, reproduced as written. Two things
 * about that copy are worth stating plainly rather than quietly smoothing
 * over, because nothing here was rewritten to hide them:
 *
 *   - The brief supplied for this page is the ChatGPT & AI Tools brief, whose
 *     later sections are themselves AI-Powered Marketing copy. So the page
 *     carries a RAG title over ChatGPT and marketing prose. That was the
 *     instruction, and it is reproduced as given.
 *   - The brief's own "Important Note for Website Use" says the ten reviews
 *     below are sample formats rather than verified testimonials, and each
 *     should be replaced with feedback from a real Techcadd student before
 *     they stand as reviews.
 *
 * Where a section of the detail page had no copy in that brief — the outcomes,
 * the eligibility checklist, the demand line and the comparison — it is
 * composed from the same brief rather than invented.
 *
...
export const ragCourse: Course = {
  ...
  /* The eight areas of "What You Will Learn", in the order the brief gives
     them. The prose behind each is in `ragLearn` below rather than
     repeated here. */
  modules: [
    {
...
export const ragSeo = {
  title: "RAG (Retrieval-Augmented Generation) Course in Mohali | Techcadd",
  description:
    "Techcadd's RAG (Retrieval-Augmented Generation) course in Mohali — hands-on AI training with live projects, a portfolio and a completion certificate. No coding background required.",
};
...
export const ragSectionCopy: Partial<
...
export const ragLearn: LearnDetail = {
  intro:
 ...
 */
export const ragWhyChoose: { icon: string; title: string; body: string }[] = [
  {
    icon: "chart",
...
export const ragAudience: { title: string; body: string; icon: string }[] = [
  {
    icon: "users",
...
export const ragFaqs: { q: string; a: string }[] = [
  {
 ...
 * details.
 */
export const ragReviews: CourseReview[] = [
  {
    name: "Student",
